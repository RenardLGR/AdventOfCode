...
function solveOne(string){
    ...
    string.split('\n').forEach(sack =>{
        ...
        let left = sack.slice(0, half)
        let right = sack.slice(half)
        for(let i=0 ; i<half ; i++){
            if(right.includes(left[i])){
                res += priorities.indexOf(left[i])
                break
            }
        }
    })
...
function solveTwo(string){
    ...
    string.split('\n').forEach(sack => {
        if(group.length === 3){
            for(let i=0 ; i<group[0].length ; i++){ // I could take any group's length, the shortest would actually be the most efficient
                if(group[1].includes(group[0][i]) && group[2].includes(group[0][i])){
                    res += priorities.indexOf(group[0][i])
                    break
                }
            }
            group = [sack]
    ...
    for(let i=0 ; i<group[0].length ; i++){ // last group
        if(group[1].includes(group[0][i]) && group[2].includes(group[0][i])){
            res += priorities.indexOf(group[0][i])
            break
        }
