...
function solveOne(signal){
    let arr = [signal[0], signal[1], signal[2], signal[3]]
    let res = 4

    while(!isAllDifferent(arr)){
        arr.shift()
        arr.push(signal[res])
        res++
    }
...
function solveTwo(signal){
    let arr = signal.slice(0, 14).split('')
    let res = 14
